...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string; icon: React.ReactNode }> = {
  pending: { label: 'Pending', color: 'text-[#D4CBBF]', bg: 'bg-[#D4CBBF]/10', icon: <Clock className="w-4 h-4" /> },
  processing: { label: 'Processing', color: 'text-[#F7F4EE]', bg: 'bg-[#F7F4EE]/10', icon: <Package className="w-4 h-4" /> },
  dispatched: { label: 'Dispatched', color: 'text-[#FAF8F5]', bg: 'bg-[#FAF8F5]/10', icon: <Truck className="w-4 h-4" /> },
  delivered: { label: 'Delivered', color: 'text-white', bg: 'bg-white/10', icon: <CheckCircle className="w-4 h-4" /> },
  ...
  return (
    <div ref={containerRef} className="max-w-6xl mx-auto space-y-6 pb-20">
      <div className="flex items-center gap-4">
        <button onClick={() => router.push('/admin/orders')} className="p-2 rounded-lg text-[#8A8478] hover:text-[#FAF8F5] hover:bg-[#181818]"><ArrowLeft className="w-5 h-5" /></button>
        <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="form-section bg-[#121212] rounded-xl border border-[#24221F] p-6">
            <h2 className="text-sm font-semibold text-[#8A8478] uppercase tracking-wider mb-4">Order Items</h2>
            <div className="space-y-3">
              {order.items?.map((item, i) => (
                <div key={i} className="flex items-center gap-4 py-3 border-b border-[#24221F]/50 last:border-0">
                  <div className="w-14 h-14 rounded-lg bg-[#1C1C1C] border border-[#24221F] overflow-hidden relative shrink-0">
                    {item.image && <Image src={item.image} alt="" fill sizes="56px" className="object-cover" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium text-[#FAF8F5]">{item.productName}</p>
                    <p className="text-xs text-[#8A8478]">EU {item.size} · {item.colour} · Qty {item.quantity}</p>
                  </div>
                  <p className="text-sm text-[#FAF8F5] font-medium">{formatPrice(item.price * item.quantity)}</p>
                </div>
              ))}
            </div>
            <div className="mt-4 pt-4 border-t border-[#24221F] space-y-2 text-sm">
              <div className="flex justify-between"><span className="text-[#8A8478]">Subtotal</span><span className="text-[#FAF8F5]">{formatPrice(order.subtotal)}</span></div>
              <div className="flex justify-between"><span className="text-[#8A8478]">Delivery — {order.deliveryAddress?.district}</span><span className="text-[#FAF8F5]">{order.deliveryFee === 0 ? 'Free' : formatPrice(order.deliveryFee)}</span></div>
              <div className="flex justify-between text-lg font-semibold pt-2 border-t border-[#24221F]"><span className="text-[#FAF8F5]">Total</span><span className="text-[#F7F4EE] font-bold">{formatPrice(order.total)}</span></div>
            </div>
          </div>

          {/* Customer */}
          <div className="form-section bg-[#121212] rounded-xl border border-[#24221F] p-6">
            <h2 className="text-sm font-semibold text-[#8A8478] uppercase tracking-wider mb-4">Customer</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
              <div><span className="text-[#8A8478]">Name</span><p className="text-[#FAF8F5] mt-0.5 font-medium">{order.customer?.name}</p></div>
              <div><span className="text-[#8A8478]">Phone</span><p className="text-[#FAF8F5] mt-0.5 font-medium">{order.customer?.phone}</p></div>
              <div><span className="text-[#8A8478]">Email</span><p className="text-[#FAF8F5] mt-0.5">{order.customer?.email || '—'}</p></div>
              <div><span className="text-[#8A8478]">Payment</span><p className="text-[#FAF8F5] mt-0.5">Cash on Delivery</p></div>
            </div>
          </div>

          {/* Delivery */}
          <div className="form-section bg-[#121212] rounded-xl border border-[#24221F] p-6">
            <h2 className="text-sm font-semibold text-[#8A8478] uppercase tracking-wider mb-4">Delivery Address</h2>
        ...
        <div className="space-y-6">
          {/* Status Actions */}
          <div className="form-section bg-[#121212] rounded-xl border border-[#24221F] p-6">
            <h2 className="text-sm font-semibold text-[#8A8478] uppercase tracking-wider mb-4">Update Status</h2>
            {nextStatuses.length > 0 ? (
              <div className="space-y-2">
                {order.status === 'pending' || order.status === 'processing' ? (
                  <div className="mb-3">
                    <label className="text-xs text-[#8A8478] mb-1 block">Tracking Number (optional)</label>
                    <input type="text" value={trackingNumber} onChange={(e) => setTrackingNumber(e.target.value)} placeholder="Enter tracking #" className="w-full bg-[#0B0B0B] border border-[#24221F] rounded-lg px-3 py-2 text-sm text-[#FAF8F5] outline-none focus:border-[#F7F4EE]" />
                  </div>
                ) : null}
                {nextStatuses.filter((s) => s !== 'cancelled').map((s) => {
                  const statusConf = STATUS_CONFIG[s]!
                  return (
                    <button key={s} onClick={() => handleStatusUpdate(s)} disabled={updating} className={cn('w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg text-sm font-semibold transition-all shadow-xs disabled:opacity-50', 'bg-[#F7F4EE] text-[#0B0B0B] hover:bg-[#FFFFFF]')}>
              ...
              <p className="text-sm text-[#8A8478]">This order is {order.status} — no further actions.</p>
            )}
          ...
          {/* Status Timeline */}
          <div className="form-section bg-[#121212] rounded-xl border border-[#24221F] p-6">
            <h2 className="text-sm font-semibold text-[#8A8478] uppercase tracking-wider mb-4">Status History</h2>
      ...
      {cancelModal && (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50">
          <div className="bg-[#141414] border border-[#24221F] rounded-xl p-6 w-full max-w-sm mx-4">
            ...
            <textarea value={cancelReason} onChange={(e) => setCancelReason(e.target.value)} rows={3} placeholder="Enter reason..." className="w-full bg-[#0B0B0B] border border-[#24221F] rounded-lg px-3 py-2 text-sm text-[#FAF8F5] outline-none focus:border-[#F7F4EE] mb-4" />
            <div className="flex gap-3">
              <button onClick={() => { setCancelModal(false); setCancelReason('') }} disabled={updating} className="flex-1 px-4 py-2 border border-[#24221F] rounded-lg text-[#FAF8F5] hover:bg-[#181818]">Keep Order</button>
